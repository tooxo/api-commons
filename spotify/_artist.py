...
class Artist:
    ...
    def get_top_tracks_by_id(artist_id: str, token: str):
        ...
        parsed_response: dict = json.loads(
            get_request_sync(url, extra_headers=build_auth_header(
                token=token)))
        return [spotify.Track.from_api_response(json.dumps(x)) for x
                in
                parsed_response["tracks"]]

    @staticmethod
    async def get_top_tracks_by_id_async(artist_id: str, token: str):
        url = f"https://api.spotify.com/v1/artists/{artist_id}/top-tracks"
        parsed_response: dict = json.loads(
            await get_request_async(url, extra_headers=build_auth_header(
                token=token)))
        return [spotify.Track.from_api_response(json.dumps(x)) for x
                in
                parsed_response["tracks"]]
    ...
    async def get_top_tracks_async(self, token: str):
        ...
        if not self.id:
            raise IncompleteObjectError
        self.top_tracks = await Artist.get_top_tracks_by_id_async(self.id,
                                                                  token)
        return self.top_tracks
    ...
    def get_albums(self, token: str):
        ...
        if not self.id:
            raise IncompleteObjectError
        self.albums = Artist.get_albums_by_id(self.id,
                                              token)
        return self.albums
    ...
    async def get_albums_async(self, token: str):
        ...
        if not self.id:
            raise IncompleteObjectError
        self.albums = await Artist.get_albums_by_id_async(self.id,
                                                          token)
        return self.albums
